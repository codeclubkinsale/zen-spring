...
class BadgeControllerIntTest {
  ...
  void shouldFindBadgeWhenValidBadgeId() {
    ResponseEntity<Badge> response =
        restTemplate.exchange("/api/badges/1", HttpMethod.GET, null, Badge.class);
  ...
  void shouldThrowNotFoundWhenInvalidBadgeId() {
    ResponseEntity<Badge> response =
        restTemplate.exchange("/api/badges/999", HttpMethod.GET, null, Badge.class);
  ...
  void shouldCreateNewBadgeWhenBadgeIsValid() {
    ...
    ResponseEntity<Badge> response =
        restTemplate.exchange("/api/badges", HttpMethod.POST, new HttpEntity<>(badge), Badge.class);
  ...
  void shouldUpdateBadgeWhenBadgeIsValid() {
    ResponseEntity<Badge> response =
        restTemplate.exchange("/api/badges/8", HttpMethod.GET, null, Badge.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
  ...
  void shouldDeleteWithValidId() {
    ResponseEntity<Void> response =
        restTemplate.exchange("/api/badges/88", HttpMethod.DELETE, null, Void.class);
