...
class AwardControllerIntTest {
  ...
  void shouldFindAwardWhenValidAwardId() {
    ResponseEntity<Award> response =
        restTemplate.exchange("/api/awards/1", HttpMethod.GET, null, Award.class);
  ...
  void shouldThrowNotFoundWhenInvalidAwardId() {
    ResponseEntity<Award> response =
        restTemplate.exchange("/api/awards/999", HttpMethod.GET, null, Award.class);
  ...
  void shouldCreateNewAwardWhenAwardIsValid() {
    ...
    ResponseEntity<Award> response =
        restTemplate.exchange("/api/awards", HttpMethod.POST, new HttpEntity<>(award), Award.class);
  ...
  void shouldUpdateAwardWhenAwardIsValid() {
    ResponseEntity<Award> response =
        restTemplate.exchange("/api/awards/8", HttpMethod.GET, null, Award.class);
    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
  ...
  void shouldDeleteWithValidId() {
    ResponseEntity<Void> response =
        restTemplate.exchange("/api/awards/88", HttpMethod.DELETE, null, Void.class);
